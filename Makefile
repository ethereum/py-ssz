...
check-git:
	# require that upstream is configured for ethereum/py-ssz
	@if ! git remote -v | grep "upstream[[:space:]]git@github.com:ethereum/py-ssz.git (push)\|upstream[[:space:]]https://github.com/ethereum/py-ssz (push)"; then \
		echo "Error: You must have a remote named 'upstream' that points to 'py-ssz'"; \
		exit 1; \
	fi
