...
def test_inherit_from_parent_without_fields():
    class Parent(ssz.Serializable):
        pass
    ...
    child = Child(0)
    assert child.field1 == 0


def test_inherit_from_parent_with_fields():
    class Parent(ssz.Serializable):
        fields = (
            ("field1", uint8),
        )
    ...
    assert parent.field1 == 0
    assert child.field1 == 1


def test_inherit_and_replace_fields():
    class Parent(ssz.Serializable):
        fields = (
            ("field1", uint8),
        )
...
def test_inherit_and_add_fields():
    class Parent(ssz.Serializable):
        fields = (
            ("field1", uint8),
        )
    ...
    parent = Parent(1)
    assert parent.field1 == 1
    with pytest.raises(AttributeError):
        parent.field2

    child = Child(2, 3)
    assert child.field1 == 2
    assert child.field2 == 3
