...
def basic_sedes_and_values_st():
    # bitlist, bitvector and bytevector are not technically basic, but we include them
    # here as our implementation does not allow them to have children
    return st.one_of(
...
#
# Convert plain Python values generated by strategies to serializable or
# hashable datatypes
#
...
def to_plain_value(serializable_or_hashable_value, sedes):
    if isinstance(sedes, (BasicSedes, Bitlist, Bitvector, ByteVector)):
        ...
        raise TypeError(f"Unsupported type: {type(serializable_or_hashable_value)}")
...
def transform_path_st(draw, hashable_value):
    if isinstance(hashable_value, HashableContainer):
        ...
        raise TypeError(f"Unsupported type: {type(hashable_value)}")
