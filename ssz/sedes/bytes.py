...
class Bytes:
    ...
    def serialize(self, val):
        ...
        if object_len >= 2 ** 32:
            raise SerializationError(
                'Object too long for its length to fit into 4 bytes after serialization',
                val
            )

        # Convert the length of bytes to a 4 bytes value
        object_len_bytes = object_len.to_bytes(4, 'big')
    ...
    def deserialize_segment(self, data, start_index):
        ...
        object_len = int.from_bytes(data[start_index:start_index + 4], 'big')
        # object_start_index is the start index of bytes object in the serialized bytes string
        object_start_index = start_index + 4
